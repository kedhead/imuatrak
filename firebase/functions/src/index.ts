...
// ---------------------------------------------------------------------------
// Club plan billing (RevenueCat).
//
// The club subscription is bought in-app (via the "club" RevenueCat offering)
// by a club owner or admin. RevenueCat entitlement "club" being active for
// that payer is the source of truth; these functions project it onto the
// payer's club doc as subscriptionStatus="active", which is what every
// client already gates ads and channel limits on. Two entry points:
//
//  - revenuecatWebhook: RevenueCat server notifications (renewals,
//    expirations, billing issues) — the long-term sync.
//  - syncClubPlan: callable invoked by the app right after a purchase or
//    restore, so the club activates instantly instead of waiting for the
//    webhook. It re-verifies against the RevenueCat REST API, so a malicious
//    client can't activate a club it hasn't paid for.
//
// Secrets (set with `firebase functions:secrets:set`):
//  - REVENUECAT_WEBHOOK_AUTH: exact Authorization header value RevenueCat is
//    configured to send (e.g. "Bearer <long random string>").
//  - REVENUECAT_SECRET_API_KEY: RevenueCat secret API key (sk_...), used for
//    the REST verification in syncClubPlan.
// ---------------------------------------------------------------------------

...
import { subscribeMessages, sendMessage, uploadMessageMedia } from "@/services/clubService";
import { useClub } from "@/services/clubStore";
import type { ClubMessage } from "@/models/club";
...
export default function ClubChatScreen() {
  ...
  useEffect(() => {
    if (!club) return;
    return subscribeMessages(club.id, setMessages);
  }, [club?.id]);
  ...
  const onSend = async () => {
    ...
    if (!content || !club || !user) return;
    setText("");
    setSending(true);
    try {
      await sendMessage(club.id, user.uid, user.displayName ?? "Member", content);
    } catch {
  ...
  const onPickMedia = async () => {
    ...
    try {
      const msg = await sendMessage(
        club.id, user.uid, user.displayName ?? "Member", "", mediaType,
      );
      // Show local URI optimistically while uploading
      setMessages((prev) => prev.map((m) =>
        m.id === msg.id ? { ...m, mediaUrl: asset.uri } : m,
      ));
      setUploadingId(msg.id);
      const url = await uploadMessageMedia(club.id, msg.id, asset.uri, mimeType);
      setMessages((prev) => prev.map((m) =>
